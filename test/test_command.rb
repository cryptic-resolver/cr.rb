...
def cr(arg, display: false)
  ...
  unless display
    `#{RbConfig.ruby} -Ilib exe/cr #{arg}`
  else
    system("#{RbConfig.ruby} -Ilib exe/cr #{arg}")
  end
end
...
=end
  ...
  # Use this test to replace old test: test_smt
  # Because the cryptic_electronics is no loner in default dictionaries
  def test_apm
    result = cr("apm")
    assert_equal(<<~EOC, result)
  \e[32mFrom: cryptic_computer\e[0m
  ...
  \e[32mFrom: cryptic_windows\e[0m
